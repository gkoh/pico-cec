...
void irq_set_enabled(int a, int b) {
  //  called from hdmi-cec-link.c -> irq_set_enabled(IO_IRQ_BANK0, true);
}
...
void IRAM_ATTR gpio_set_irq_enabled(uint gpio, uint32_t event_mask, bool enabled) {
  ...
  if (enabled) {
    if (event_mask == (GPIO_IRQ_EDGE_RISE | GPIO_IRQ_EDGE_FALL)) {
      //ESP_LOGD(TAG, "GPIO_INTR_ANYEDGE");
      gpio_set_intr_type(gpio, GPIO_INTR_ANYEDGE);
    }
    if (event_mask == (GPIO_IRQ_EDGE_RISE)) {
      //ESP_LOGD(TAG, "GPIO_INTR_POSEDGE");
      gpio_set_intr_type(gpio, GPIO_INTR_POSEDGE);
    }
    if (event_mask == (GPIO_IRQ_EDGE_FALL)) {
      //ESP_LOGD(TAG, "GPIO_INTR_NEGEDGE");
      gpio_set_intr_type(gpio, GPIO_INTR_NEGEDGE);
    }
  } else {
    //ESP_LOGD(TAG, "GPIO_INTR_DISABLE");
...
//#define USE_GPIO_TASK_HANDLER // enable this to get the tx frame handling out of the interrupt context (for debugging)
...
//      hdmi_rx_frame_callback(io_num, io_num);  //  the second parameter will simply be sent back via
    }
  }
}
...
static void IRAM_ATTR gpio_isr_handler(void* arg) {
  ...
  if (edge_time - prev_edge_time < 100) return;
...
#ifdef USE_GPIO_TASK_HANDLER
  //create a queue to handle gpio event from isr (we could have just used a global for the timestamp and an ipc signal)
  gpio_evt_queue = xQueueCreateStatic(GPIO_QUEUE_LENGTH, sizeof(uint64_t), &storageGPIOQueue[0], &xStaticGPIOQueue); // TODO: changed from xQueueCreate to xQueueCreateStatic
  if (!gpio_evt_queue) {
    ESP_LOGE(TAG, "Creating GPIO queue failed");
    return;
  }
  //start gpio task
  xGPIOTask = xTaskCreateStatic(gpio_task, "gpio_task", GPIO_STACK_SIZE, &gpio_evt_queue, configMAX_PRIORITIES - 1, &stackGPIO[0], &xGPIOTCB); // TODO: ditto
  (void)xGPIOTask;
#endif // USE_GPIO_TASK_HANDLER
...
//    ESP_LOGD(TAG, "Started timer, next: %lld us", next);
    //ESP_LOGD(TAG, "Started timer, delay: %ld us", (int32_t)(time_since_boot - last_timer_value));
  } else if (next < 0) {
    ...
    ESP_LOGV(TAG, "%s frame complete: %ld us", frame_type_str, (uint32_t)(time_since_boot - frame_time_start));
  }
...
alarm_id_t IRAM_ATTR add_alarm_at(absolute_time_t time, alarm_callback_t callback, void *user_data, bool fire_if_past) {
...
There is no such limitation for level-triggered interrupts, which means, if there are no edge-triggered interrupts configured within a group, then there can be any number of level-triggered interrupts in that group.
...
After the CPU services the interrupt, change the GPIO interrupt type to low. A second interrupt occurs at this time, and the CPU needs to ignore the interrupt service routine.
To trigger a GPIO interrupt on a falling edge, follow the steps:
Set the GPIO interrupt type to low.
After the CPU services the interrupt, change the GPIO interrupt type to high. A second interrupt occurs at this time, and the CPU needs to ignore the interrupt service routine.
